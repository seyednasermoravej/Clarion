...
func clientSim(msgType, msgBlocks int, pubKeys []*[32]byte, messagingMode bool) ([]byte, time.Duration) {
	...
	bodyShares := SplitSecret([]byte(msg), numServers-1, numServers)

	msgShamir := ReconstructSecret(bodyShares[:numServers-1])

	msgBytes := []byte(msgShamir)

	msgDe := mycrypto.DecryptCT(msgBytes)
	log.Print(msgDe)
